...
def send_packet_unreliable(sender_socket, packet, receiver_addr):
    if random.random() < 0.9:  # 10% probability of corruption
        sender_socket.sendto(packet, receiver_addr)
    else:
        print("\t\tPacket lost artificially from sender side")
...
def main():
    ...
    while True:
        ...
        send_packet_unreliable(sender_socket, packet, receiver_addr)
